...
interface DeleteCandidateBenchProps {
  closePopup: () => void;
  CandidateBenchData: {
    id: number;
  };
...
export const DeleteCandidateBenchPopup: React.FC<DeleteCandidateBenchProps> = ({
  closePopup,
  CandidateBenchData,
  refreshData
}) => {
  ...
  const handleAgentDelete = async () => {
    ...
    try {
      const success = await deleteJobPosting(CandidateBenchData.id);
      if (success) {
        closePopup();
        refreshData();
        toast.success("Candidate Bench Deleted Successfully")
      } else {
        setError("Failed to delete Candidate Bench. Please try again.");
        toast.error("Failed to delete Candidate Bench. Please try again.")
      }
    } catch (error: any) {
      setError(error.message || "Failed to delete Candidate Bench. Please try again.");
      toast.error("Failed to delete Candidate Bench. Please try again.");
    } 
  };
      ...
      <div className="container mx-auto">
        <div className="relative bg-white rounded-[5px] w-4/12 mx-auto px-5 py-5">
          <div className="relative mb-10">
            <h2 className="text-2xl text-armsBlack font-semibold pb-3 border-b-2 border-armsgrey ">Delete Candidate Bench</h2>
